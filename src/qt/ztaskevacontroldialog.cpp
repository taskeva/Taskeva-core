...
#include "ui_ztaskevacontroldialog.h"
...
std::list<std::string> ZTaskevaControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZTaskevaControlDialog::listMints;

ZTaskevaControlDialog::ZTaskevaControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZTaskevaControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZTaskevaControlDialog::~ZTaskevaControlDialog()
{
    delete ui;
}

void ZTaskevaControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZTaskevaControlDialog::updateList()
{
...
void ZTaskevaControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZTaskevaControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZTaskeva_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZTaskevaControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZTaskevaControlDialog::GetSelectedMints()
{
...
void ZTaskevaControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
