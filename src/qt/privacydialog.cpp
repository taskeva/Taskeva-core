...
#include "ztaskevacontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zTASK ought to be enough for anybody." - Bill Gates, 2017
    ui->zTaskevapayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzTaskevaSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzTaskeva->setEnabled(false);
        ui->pushButtonMintzTaskeva->setToolTip(tr("zTASK is currently disabled due to maintenance."));

        ui->pushButtonSpendzTaskeva->setEnabled(false);
        ui->pushButtonSpendzTaskeva->setToolTip(tr("zTASK is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zTaskevapayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzTaskeva_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTASK is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zTASK...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zTASK in ") +
...
void PrivacyDialog::on_pushButtonSpendzTaskeva_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTASK is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zTASK
        sendzTaskeva();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zTASK
    sendzTaskeva();
}

void PrivacyDialog::on_pushButtonZTaskevaControl_clicked()
{
    ZTaskevaControlDialog* zTaskevaControl = new ZTaskevaControlDialog(this);
    zTaskevaControl->setModel(walletModel);
    zTaskevaControl->exec();
}

void PrivacyDialog::setZTaskevaControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzTaskevaSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzTaskeva()
{
    QSettings settings;
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Taskeva Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zTaskevapayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zTASK is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " TASK </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zTASK</b>";
    ...
    // use mints from zTASK selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZTaskevaControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZTaskevaControlDialog::GetSelectedMints();
    }
    ...
    // Clear ztaskeva selector in case it was used
    ZTaskevaControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zTASK Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Taskeva, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zTASK Mint");
    ...
    // Clear amount to avoid double spending when accidentally clicking twice
    ui->zTaskevapayAmount->setText ("0");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zTASK </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zTASK "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zTASK "));
    ui->labelzTaskevaAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzTaskevaSyncStatus->setVisible(fShow);
}
